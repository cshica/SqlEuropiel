...
GO
/****** Object:  StoredProcedure [dbo].[recupera_monitoreo_callcenter_solo_resumen2]    Script Date: 16/06/2021 10:47:45 a. m. ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROCEDURE [dbo].[recupera_monitoreo_callcenter_solo_resumen2]
AS
BEGIN
	DECLARE
		@fechahoy				        DATETIME,
    ...
    FROM
        @InfoCC
    WHERE
        estatus = 'Colgó' AND duracion_espera >= '00:00:01'
        AND hoy = 1
	...
	SELECT
		@entro_hoy		= SUM(hoy),
		@entro_sem		= SUM(semana),
		@entro_quin		= SUM(i.quincena),
		@entro_mes		= COUNT(*)
	FROM
		@InfoCC i
	WHERE
    ...
    SELECT
        @total_hoy      = @colgo_hoy + @entro_hoy,
        @total_sem      = @colgo_sem + @entro_sem,
		@total_quin		= @colgo_quin + @entro_quin,
        @total_mes      = @colgo_mes + @entro_mes
    ;

	SELECT 'tblDatos' Result,
		total_global_hoy		    = @entro_hoy,
		total_global_semana		    = @entro_sem,
		total_global_quincena		= @entro_quin,
		total_global_mes		    = @entro_mes,
		promedio_global_mes		    = '00:00',
		promedio_mes_espera		    = '00:00',
		llamadas_5om_minutos	    = @colgo_hoy,
        llamadas_espera_3om_semana  = @colgo_sem,
		total_colgo_quincena		= @colgo_quin,
        llamadas_espera_3om_limite  = @colgo_mes,
        total_hoy                   = @total_hoy,
        total_sem                   = @total_sem,
		total_quin					= @total_quin,
        total_mes                   = @total_mes
	;
