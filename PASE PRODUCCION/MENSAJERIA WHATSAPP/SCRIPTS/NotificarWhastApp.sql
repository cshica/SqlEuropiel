...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	*/ 
		...
		DECLARE @HORA_EJECUCION DATETIME=(SELECT MAX(HORA_EJECUCION) FROM TABLA_NOTIFI_WHATSAPP (nolock))
		...
		BEGIN
			...
			inner join TABLA_NOTIFI_WHATSAPP (nolock) t on t.id_cita=tt.id_cita
			group by tt.id_cita
			)
		END
...
--SI SE EJECUTA A LAS 00 HORAS, SIGUE CONSIDERANDO EL DIA ACTUAL COMO FECHA, POR LO TANTO LE SUMAMOS UNDIA PARA QUE LA FECHA DE ENVIO SEA JUSTO AL DIA SIGUIENTE A PRIMERA HORA
IF CAST(@HORA_EJECUCION AS TIME) =cast('00:00:00.000' as time)
BEGIN
	--SET  @dia_hora_inicio  =( DATEADD(HOUR,DATEPART(hour,@hora_inicio), cast(@FECHA_INICIO as datetime)+1))

	UPDATE #NUEVA_TABLA SET envio_confirmar=envio_confirmar+1 WHERE cast(HORA_EJECUCION as date)=cast(GETDATE() as date)

END
/*********************************************************************************************************
POR ALGUN MOTIVO, LA TABLA NOTIFI_WHATSAPP LLENA CITAS QUE NO EXISTEN EN LA TABLA CITA
...
fecha_confirmacion=GETDATE()
,enviado=1,recordado1=1,recordado2=1
,tipo_confirmacion='CANCELADO(SIS)'--CANCELADOS POR EL SISTEMA
,confirmada=0
from TABLA_NOTIFI_WHATSAPP TN
