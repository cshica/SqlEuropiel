...
select  *  into #tabla from Paso2_Junio where   From_ in( select t.NumeroWhatsapp from WhatsappEmisor t)
--------------------------------------------------------------------------------------------------------------------------------------------
-- PASO 2
-- Obenemos todos los mensajes para elegir uno por uno los FILTROS a buscar
SELECT Body FROM #tabla where Body like '%Felicidades%_has%' ORDER BY Body ASC
--Tu OTP es 00184413
--------------------------------------------------------------------------------------------------------------------------------------------
...
--PASO 3
declare @id int =1
declare @filtro nvarchar(max)='%Felicidades%_Has sido%'
DECLARE @ENVIOS INT
